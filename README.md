...
### `public/.nojekyll` — kept, but it is not what protects `_astro/`

An empty file, worth keeping and worth not misunderstanding. Jekyll excludes
every directory whose name starts with an underscore, which is all of `_astro/`
— but Jekyll only runs under the legacy Pages source, *Deploy from a branch*.
This repository deploys through `actions/deploy-pages`, which serves the
uploaded artifact verbatim and never invokes Jekyll.

So `_astro/` is protected by the **Pages source setting**, not by this file.
Keep the file anyway: it costs nothing and it is what makes the site survive
someone flipping the source to a branch. Do not treat its presence as the
guarantee — the guarantee is Settings → Pages → Source → GitHub Actions.
...
**Eight** things carry the hostname outside the page HTML, and only one of them
is `site`. This list said five until the move to `steejaarjun.github.io`, and
the three it omitted were the three that survive `site` being changed — which is
exactly the shape of a half-finished move. Run `npm run verify` after working
through it; the script exists because a checklist nobody re-reads is not a
control.

Outside the repository. Neither of these is fixed by any commit, and both are
still wrong immediately after a move:
...
2. **Edge function CORS** — `ALLOWED_ORIGINS` in
   ...
   Editing without redeploying changes nothing. Verify without writing a row:

   ```bash
   curl -si -X OPTIONS https://<project-ref>.supabase.co/functions/v1/rsvp \
     -H "Origin: https://steejaarjun.github.io" \
     -H "Access-Control-Request-Method: POST" | grep -i access-control-allow-origin
   ```

   A missing `Access-Control-Allow-Origin` in the response means the origin is
   not on the deployed list — the preflight still returns 204 either way, so the
...
4. **`src/layouts/Base.astro`** — the `Astro.site ?? new URL(…)` fallback. Item 3
   does **not** cover it. It is what every absolute URL falls back to when `site`
   is absent, so it fails only in the case nobody tests.
5. **`src/pages/index.astro`** — the second copy of that same fallback.
6. **`public/robots.txt`** — the `Sitemap:` line is absolute and is the one URL
   in the project not derived from `site`. The `Disallow:` path changes with the
   base.
7. **`public/site.webmanifest`** — `start_url`, `scope` and three icon paths.
   Generated by `scripts/make-brand-assets.mjs`, which carries its own copy of
   the deploy root; keep the two in step.
8. **`scripts/check-links.mjs`** — the `SITE` constant. Left stale, the verifier
   treats every in-site absolute URL as an external host and reports a clean
   build while checking almost nothing.
